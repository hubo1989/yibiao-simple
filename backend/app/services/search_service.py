...
class SearchService:
    ...
    def __init__(
        self, max_results: int = 5, safe_search: str = "moderate", region: str = "cn"
    ):
        """
        初始化搜索服务
    ...
    def _extract_content_with_requests(
        ...
        try:
            ...
            logger.info(
                f"成功读取网页内容: {url}, 标题: {title}, 内容长度: {len(content)}"
            )
    ...
    async def _extract_with_playwright(
        ...
        try:
            async with async_playwright() as p:
                ...
                logger.info(
                    f"Playwright成功读取: {url}, 标题: {title}, 内容长度: {len(content)}"
                )
    ...
    def _extract_with_seleniumbase(self, url: str, max_chars: int) -> Dict[str, str]:
        ...
        try:
            with Driver(uc=True, headless=True) as driver:
                ...
                logger.info(
                    f"SeleniumBase成功读取: {url}, 标题: {title}, 内容长度: {len(content)}"
                )
        ...
        if PLAYWRIGHT_AVAILABLE:
            try:
                ...
                result = loop.run_until_complete(
                    self._extract_with_playwright(url, max_chars)
                )
                return result
            except Exception as e:
    ...
    async def load_url_content_async(
        self, url: str, max_chars: int = 5000
    ) -> Dict[str, str]:
        """
        异步读取链接内容
